  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`${name} đã cố gỡ 1 tin nhắn nhưng gỡ thế nào nhanh bằng Pò cute được :3\nNội dung: ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`${name} vừa gỡ ${getMsg.attachment.length} tệp đính kèm.${(getMsg.msgBody != "") ? `\n\nNội dung: ${getMsg.msgBody}` : ""}`,
              attachment:[],
